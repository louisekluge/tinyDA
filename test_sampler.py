...
def validate_sample_dict_da(out, expected_sampler, n_chains, iterations, subchain_length):
    ...
    for i in range(n_chains):
        key = f"chain_promoted_coarse_{i}"
        assert key in out
        chain = out[key]
        assert len(chain) == iterations + 1
        assert isinstance(chain, list)
...
#--------------------------------------------------------------------------------------------
# test sample() returned values
def validate_sample_dict_mlda(out, expected_sampler, n_chains, iterations, subchain_lengths, randomize_subchain_length):
    # Check metadata (info)
    assert out["sampler"] == expected_sampler
    assert out["n_chains"] == n_chains
    assert out["iterations"] == iterations + 1
    ...
    assert out["randomize_subchain_length"] == randomize_subchain_length
    ...
    for j in range(n_chains):
        for i in range(2):
            key = f"promoted_l{i}_{j}"
            assert key in out
            chain = out[key]
            if i==0:
                assert len(chain) == iterations * 2
            if i==1:
                assert len(chain) == iterations
            assert isinstance(chain, list)
            for link in chain:
                assert isinstance(link, Link)
...
def test_mlda_sampling(posteriors, proposal_mlda, iterations, n_chains):
    ...
    validate_sample_dict_mlda(result_mlda, 'MLDA', n_chains=n_chains, iterations=iterations, subchain_lengths=[2,2], randomize_subchain_length=False)
